...
/*   Updated: 2024/08/13 14:15:16 by sbartoul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

t_node	*ft_combine(t_token_type id, t_node *left,
	t_node *right, t_minishell *g)
{
	t_node	*node;
...
t_node	*ft_expression(int i, t_minishell *g)
{
	...
	while (ft_is_token_sep(g) && ft_token_prec(g) >= i)
	{
		...
		if (!left)
			return (clear_astree(&left, g), clear_astree(&right, g), NULL);
	}
	return (left);
}
